...
# Model Description Tab
    ...
    with tab3:
        ...
        with st.sidebar.container():
            preg = st.slider('Pregnancies', 0, 17, 3, 1, help="Number of times the patient has been pregnant.")
            glucose = st.slider('Glucose', 0, 199, 117, 1, help="Plasma glucose concentration after a 2-hour oral glucose tolerance test.")
            bp = st.slider('Blood Pressure', 0, 122, 72, 1, help="Diastolic blood pressure (mm Hg).")
            skinthickness = st.slider('Skin Thickness', 0, 99, 23, 1, help="Triceps skin fold thickness (mm).")
            insulin = st.slider('Insulin', 0, 846, 30, 1, help="2-Hour serum insulin (mu U/ml).")
            bmi = st.slider('BMI', 0.0, 67.1, 32.0, 0.1, help="Body mass index (weight in kg/(height in m)^2).")
            dpf = st.slider('Diabetes Pedigree Function', 0.078, 2.42, 0.3725, 0.001, help="A function which scores likelihood of diabetes based on family history.")
            age = st.slider('Age', 21, 81, 29, 1, help="Age of the patient (years).")
...
with st.sidebar:
    selected_model = option_menu(
        "Main Menu", 
        ["Home", "SVM", "Linear Regression", "Random Forest", "KNN"], 
        icons=['house', 'box', 'layers', 'tree', 'geo-alt'], 
